$(function(){
  ...
  let skillsHeader = 'Brain Power';
    let skillsTestimonial = ['<p class="quote">','Will\’s ability to communicate complex topics is unbelievably clear and concise. If anyone can laser target the topic at hand, it\'s Will. Down the road, it\'s a no brainer that he will be in a position of leadership because of his natural intelligence and ability to communicate. When Will would work with his fellow students, he naturally took the role of the group leader, and as a result, his incredible ability to plan before aimlessly jumping into execution, combined with the communication of those plans made not only himself, but the other developers on his team able to produce work that was at much higher quality than if they were to just jump in. I would recommend will for a company that wants a long term employee, someone that they can take and grow internally, with the eventual goal of senior developer. A team that has Will is bound to be a successful one.', '</p>', '<br>', '<p class="source">', '- Dirk Dunn, General Assembly Instructor', '</p>', '<hr>'].join('');
      ...
      let skillShorthandOne = ['<strong>', 'TL;DR ', '</strong>', '<span class="tldr">', 'I\’m a creative, meticulous and methodical developer that loves the process of creating products. And I\'m a humanist goofball.', '</span>'].join('');
      let skillShorthandTwo = ['<strong>', 'TL;DR ', '</strong>', '<span class="tldr">', 'Vanilla Javascript or jQuery will do most of the time but for larger, data driven applications React is the jam. I prefer functional programming over OO programming.', '</span>'].join('');
      let skillShorthandThree = ['<strong>', 'TL;DR ', '</strong>', '<span class="tldr">', 'I\’ve self-studied design paradigms for several years and am keenly aware of how important UX is to the success of an app.', '</span>'].join('');
      let skillShorthandFour = ['<strong>', 'TL;DR ', '</strong>', '<span class="tldr">', 'I\’m a highly creative digital craftsman that finds unique ways to bring captivating worlds to life.', '</span>'].join('');
  ...
  let projectsHeader = 'All Together Now';
    let projectsTestimonial = ['<p class="quote">','Will combines careful planning with brilliant execution. I had the pleasure of working with him and can confidently say whether he is working as the product manager, designer, or fellow developer, he brings a terrific work ethic, strong communication, and integrity. I cannot recommend him highly enough.', '</p>', '<br>', '<p class="source">', '- Gina Fitzgerald, Muttmatch Team', '</p>', '<hr>'].join('');
      ...
      let projectOneHeader = ['<div class="header-container">', '<img class="project-icon" src="images/muttmatch-icon.png" alt="Muttmatch Icon" />', '<a href="https://will-sparger.github.io/muttmatch_frontend/" target="_blank" title="Go to Site">', '<h3 class="project-header first">', 'Muttmatch >', '</h3>','</a>', '</div>'].join('');
      let projectTwoHeader = ['<div class="header-container">', '<img class="project-icon" src="images/gistalt-icon.png" alt="gistAlt Icon"/>', '<a href="https://gistalt-96586.firebaseapp.com" target="_blank" title="Go to Site">', '<h3 class="project-header second">', 'gistAlt >', '</h3>', '</a>', '</div>'].join('');
  ...
  let homeContent = ['<p>', homeParagraphOne, '</p>',
                      '<p>', homeParagraphTwo, '</p>'].join('');
  let bioContent = [bioTestimonial,
                    '<p>', bioParagraphOne , '</p>',
                      '<p>', bioParagraphTwo, '</p>'].join('');
  let skillsContent = [skillsTestimonial,
                        '<h3 class="subheader">', skillHeaderOne, '</h3>',
                          '<p>', skillShorthandOne, '</p>',
                            '<p class ="content-paragraph">', skillParagraphOne, '</p>',
                              '<h3 class="subheader">', skillHeaderTwo, '</h3>',
                                skillListTwo,
                                  '<p>', skillShorthandTwo, '</p>',
                                    '<p class ="content-paragraph">', skillParagraphTwo, '</p>',
                                      '<h3 class="subheader">', skillHeaderThree, '</h3>',
                                        skillListThree,
                                          '<p>', skillShorthandThree, '</p>',
                                            '<p class ="content-paragraph">', skillParagraphThree, '</p>',
                                              '<h3 class="subheader">', skillHeaderFour, '</h3>',
                                                skillListFour,
                                                  '<p>', skillShorthandFour, '</p>',
                                                    '<p class ="content-paragraph">', skillParagraphFour, '</p>'].join('');
  let projectsContent = [projectsTestimonial,
                          projectOneHeader,
                            projectOneTech,
                              '<p class="content-paragraph">', projectOneParagraph, '</p>',
                                '<h3>', projectTwoHeader, '</h3>',
                                  projectTwoTech,
                                    '<p class="content-paragraph">', projectTwoParagraph, '</p>',
                                      '<h3>', projectThreeHeader, '</h3>',
                                        projectThreeParagraph].join('');
  let currentContent = ['<p>', currentShorthand, '</p>',
                          '<h3 class="subheader">', currentSubheaderOne, '</h3>',
                            '<p>', currentParagraphOne, '</p>',
                              '<p>', currentParagraphTwo, '</p>',
                                '<h3 class="subheader">', currentSubheaderTwo, '</h3>',
                                  '<p>', currentParagraphThree, '</p>',
                                    '<h3 class="subheader">', currentSubheaderThree, '</h3>',
                                      '<p>', currentParagraphFour, '</p>',
                                        currentQuote,
                                          '<h3 class="subheader">', currentSubheaderFour, '</h3>',
                                            '<p>', currentParagraphFive, '</p>',
                                              '<p>', currentParagraphSix, '</p>'].join('');
  let artworkContent = artworkParagraphOne;
  let contactContent = [contactParagraphOne,
                          contactQuote].join('');
  ...
  //   if ($(window).width() > 809 && navLinks[1].style.flex-direction === 'column') {
  //     hideNavItems();
  //   }
  // });

  let Nav = ['<nav id="nav-list" class="nav-options">',
                    '<a href="" id="menu-button" class="burger" title="Navigation Options">', '&#9776;', '</a>',
                    '<a href="" id="home"  title="Home">', 'home', '</a>',
                    '<a href="" id="bio"  title="How I got here">', 'bio', '</a>',
                    '<a href="" id="skills" title="What I can do">', 'skills', '</a>',
                    '<a href="" id="projects" title="Projects at GA">', 'projects', '</a>',
                    '<a href="" id="current" title="What I\'m working on">', 'current', '</a>',
                    '<a href="" id="artwork" title="Samples of my digital art">', 'artwork', '</a>',
                    '<a href="" id="contact" title="Let\'s collaborate!">', 'contact', '</a>',
                  '</nav>'].join('');
  $('#nav').html(Nav)
  ...
  let menuAction = function(e) {
    ...
    //console.log('display is: ', navLinks[1].id);
